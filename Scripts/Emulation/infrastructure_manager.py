...
def deploy_infrastructure(emul_type, fileADir):
	...
	with open(fileADir, "r") as json_file:
		...
		for node in nodes:
			...
			print ("Bridge sw{} is up for the {} node!".format(node["nodeNumber"], node["nodeType"]))
			...
			if(node["nodeType"] == "Core" or node["nodeType"] == "CU" or node["nodeType"] == "DU" or node["nodeType"] == "RU"):
				...
				print ("Bridge tor{} is up for the {} node!".format(node["nodeNumber"], node["nodeType"]))
				...
				print("-----sw{} connected to tor{}-----".format(node["nodeNumber"], node["nodeNumber"]))
