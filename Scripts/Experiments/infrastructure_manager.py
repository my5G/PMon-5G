						...
						with open("{}/pmon{}_{}.cfg".format(directory, node["nodeNumber"], vm["vmNumber"]), "w+") as new_cfg:
							...
							for line in old_cfg:
								...
								if(line.rstrip() == CPU_string):
									...
									new_cfg.write(line.replace(bridge_string, "vif = ['script=vif-openvswitch, bridge=br-exp-ran', 'script=vif-openvswitch,bridge=tor{}', 'bridge=br_labora']".format(node["nodeNumber"])))#the br-pmon bridge needs to be in the last position, so it can be attached to the eth2 inside the VM's
								else:
									new_cfg.write(line)
		...
		for node in nodes:
			...
			if(node["nodeType"] == "Core" or node["nodeType"] == "CU" or node["nodeType"] == "DU" or node["nodeType"] == "RU"):
				...
				for vm in vms:
					...
					stdin, stdout, stderr = ssh.exec_command('ifconfig eth2 10.16.100.{} netmask 255.255.0.0'.format(Node_IP)) #commands to bring up the interface that talks to the internet from the br-pmon bridge
					...
					#Opening another SSH session to clean firewall.
					VMip = '169.254.{}.{}'.format(node["nodeNumber"], vm["vmNumber"])
					ssh = paramiko.SSHClient()
					ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
					ssh.connect(VMip, username='root', password='necos')
					stdin, stdout, stderr = ssh.exec_command('sudo /sbin/iptables-restore < /etc/iptables/rules.v4')
					stdin, stdout, stderr = ssh.exec_command('sudo /sbin/ip6tables-restore < /etc/iptables/rules.v6')
					ssh.close()
