		...
		for node in nodes:
			...
			if(node["nodeType"] == "Core" or node["nodeType"] == "CU" or node["nodeType"] == "DU" or node["nodeType"] == "RU"):
				...
				for vm in vms:
					...
					stdin, stdout, stderr = ssh.exec_command('ifconfig eth2 200.137.197.{} netmask 255.255.255.0 && route add default gw 200.137.197.193 && dhclient eth2'.format(Node_IP)) #commands to bring up the interface that talks to the internet from the br-pmon bridge
