		...
		for service in services:
			...
			for flow in flows:
					...
					for node in nodes:
						...
						if(emul_type == "vran"):
							...
							if(node["nodeNumber"] == flow_client):
								...
								VMclientip = '172.16.{}.{}'.format(node["nodeNumber"], vm_client["vmNumber"])
							...
							if(node["nodeNumber"] == flow_server):
								...
								VMserverip = '172.16.{}.{}'.format(node["nodeNumber"], vm_server["vmNumber"])
				...
				if(emul_type == "vran"):
					...
					#source_ip = '169.254.{}.{}'.format(vm_client["vmNumber"], ctn_client)
					#target_ip = '169.254.{}.{}'.format(vm_server["vmNumber"], ctn_server)

					source_ip = '172.16.{}.{}'.format(vm_client["vmNumber"], ctn_client)
					target_ip = '172.16.{}.{}'.format(vm_server["vmNumber"], ctn_server)
					...
					stdin, stdout, stderr = ssh.exec_command('iperf -s -u -e -i 1 > server{}.txt"'.format(flows[i]["flowIdentifier"]))
					time.sleep(0.5)
					ssh.close()
					...
					stdin, stdout, stderr = ssh.exec_command('iperf -c 169.254.{}.{} -u -b {}m -e --tos {} -i 1 -t {} > client{}.txt"'.format(vm_server["vmNumber"], ctn_server, bandwidth, tos, int(duration), flows[i]["flowIdentifier"]))
					time.sleep(0.5)
					ssh.close()
					...
					source_ip = '172.16.{}.{}'.format(vm_server["vmNumber"], ctn_server)
					target_ip = '172.16.{}.{}'.format(vm_client["vmNumber"], ctn_client)
